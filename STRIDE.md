...
- ASiC-E completeness reporting: data ZIP entries absent from the ASiCManifest are reported via `AsicVerifyResult.UnreferencedFileNames` and an informational `Manifest completeness` step; `Extract` and `ExtractAll` return only manifest-referenced files (see T-9)
