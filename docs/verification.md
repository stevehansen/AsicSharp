...
- **`Verify(Stream)` and `Verify(byte[])` must produce step-for-step identical results.** Both run one private `VerifyContainer(Func<ZipArchive>)`; the factory exists so a malformed container becomes a failed result from inside the same try block whichever overload was called. `Verify_StreamOverload_ShouldMatchTheByteArrayOverload` compares name, verdict and detail of every step.
- **ASiC-E manifest digests are computed straight from the archive** via `ComputeEntryHash`, so verifying an ASiC-E container never inflates a data file. Consequence: the `Data file: <uri>` step reports `entry.Length` — the ZIP's declared size — rather than the byte count actually read. Identical for any well-formed container; an entry whose central directory misstates its size fails its digest check anyway.
- **The ASiC-S path is deliberately not streaming.** It still reads the data file into `AsicVerifyResult.DataBytes`, which that profile's result contract promises. `Verify(Stream)` therefore avoids the container copy on both profiles, but avoids the data-file copy only on ASiC-E.
